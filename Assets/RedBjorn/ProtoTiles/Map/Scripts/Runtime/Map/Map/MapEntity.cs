...
namespace RedBjorn.ProtoTiles
{
    ...
    public partial class MapEntity : IMapNode, IMapDirectional
    {
        ...
        Func<Vector3Int, int, List<Vector3Int>> AreaFunc;

        Action<Vector3Int, TileDictionary, int> Reset;
        ...
        public List<TileEntity> LineCast(Vector3 worldPosStart, Vector3 worldPosFinish, float range, Func<TileEntity, bool> condition, Func<TileEntity, bool> includeLast)
        {
            ...
            var distance = Distance(worldPosStart, worldPosFinish);
            var step = (worldPosFinish - worldPosStart) / distance;
            var d = Mathf.Max(distance, range);
            for (int i = 0; i <= d; i++)
            {
                var tile = Tile(worldPosStart + step * i);
                if (tile != null)
        ...
        {
            var nodes = NodePathFinder.Path(this, Tile(from), Tile(to), (int)range);
        ...
        public List<Vector3> WalkableBorder(TileEntity origin, float range)
        {
            ...
            var borderedAreas = MapBorder.FindSidePositions(this, walkable);
        ...
        public List<Vector3> Area(Vector3 origin, float range)
        {
            ...
            if (tile != null)
            {
                var areaTiles = AreaFunc(tile.Position, (int)range);
        ...
        {
            var cellPrefab = Settings.CreateCell();
        ...
        int IMapNode.Distance(INode x, INode y)
        {
        ...
        void IMapNode.Reset(int range, INode startNode)
        {
            if (startNode != null)
            {
