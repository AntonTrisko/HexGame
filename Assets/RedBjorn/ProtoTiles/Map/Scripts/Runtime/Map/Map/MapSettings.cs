...
#pragma warning restore 0414
        ...
        /// </summary>
        Func<Vector3Int, Vector3Int, int> DistanceFunc
        {
            get
            {
                switch (Type)
        ...
        void MarkAreas(Func<Vector3Int, Vector3Int, int> DistanceFunc, Vector3Int[] NeighboursDirection)
        {
        ...
        void ResetHex(Vector3Int startPos, TileDictionary Tiles, int range)
        {
        ...
        void ResetSquare(Vector3Int startPos, TileDictionary Tiles, int range)
        {
            for (int x = -range; x <= range; x++)
            {
        ...
        public GameObject CreateCell()
        {
            switch (Type)
            {
                case GridType.Hex: return CreateCellHex(Edge, BorderSize);
                case GridType.Square: return CreateCellSquare(Edge, BorderSize);
            }
            return null;
        }

        GameObject CreateCellHex(float size, float border)
        {
        ...
        GameObject CreateCellSquare(float size, float border)
        {
            var go = new GameObject("Cell");
            var mesh = new Mesh();
            var vertices = new List<Vector3>();
